...
@section('title', 'Add Suspension')

@section('content')
<div class="card">
    <div class="card-header">
        <i class="bi bi-pause-circle me-2"></i>Add New Suspension
    </div>
    <div class="card-body">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <select name="reason" class="form-select @error('reason') is-invalid @enderror">
                        <option value="non_payment" {{ old('reason') == 'non_payment' ? 'selected' : '' }}>Non Payment</option>
                        <option value="request" {{ old('reason') == 'request' ? 'selected' : '' }}>Customer Request</option>
                        <option value="violation" {{ old('reason') == 'violation' ? 'selected' : '' }}>Violation</option>
                ...
                <div class="col-md-6">
                    ...
                    <select name="status" class="form-select @error('status') is-invalid @enderror">
                        ...
                        <option value="reconnected" {{ old('status') == 'reconnected' ? 'selected' : '' }}>Reconnected</option>
                    </select>
                ...
                <div class="col-md-6">
                    ...
                    <input type="date" name="suspension_date" class="form-control @error('suspension_date') is-invalid @enderror"
                        value="{{ old('suspension_date', date('Y-m-d')) }}">
                ...
                <div class="col-md-6">
                    ...
                    <select name="actioned_by" class="form-select">
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ old('actioned_by') == $user->id ? 'selected' : '' }}>
                                {{ $user->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-12">
                    <button type="submit" class="btn btn-primary">
                        <i class="bi bi-check-lg"></i> Save Suspension
                    </button>
